...
- For a very big file, cut it in *Rows to read* first - a date filter or a top N - and keep the Parquet snapshot on. `COMPARE_DUCKDB_MEMORY` caps DuckDB's memory when the machine is shared, and with it the table the profile and the key search hold in memory.
