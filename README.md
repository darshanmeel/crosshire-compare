...
3. Press **Figure it all out and compare** - the same verdict as the file route, and the files are `A_SAMPLE_compare_B_SAMPLE__*`, because a database side left at its default name takes the connection's name - and when both sides take the same one, the tag tells them apart.
...
**Suggest keys** finds the combinations of up to four columns that identify a row on both sides - with Desbordante when it is installed (HyUCC for exact keys on the first 200,000 rows of the side counted, verified on every row; PyroUCC's almost-unique combinations when nothing is exact), otherwise a level at a time: every column on its own, then every pair of the most key-like columns, then combinations of three, then of four - each level only when the one before found no key. Every candidate gets its **Overlap %** - the share of the Left side's distinct key values that exist on the Right, which is what a key is for - and a **Why**: one line of plain reasons. *name says identifier · no nulls · 3,000 distinct of 3,000 in HR, 2,985 of 2,985 in Payroll · 98.7% of HR's values found in Payroll · unique by itself*; or *salary: a measure, decimal - never a key*; or *no values in common - none of HR's hire_date values found in Payroll* for a key that pairs nothing because one column is spelled differently. A level that finds a key ends the search: a column unique by itself is never combined with anything and nothing is looked for beside it - `emp_id` is never followed by `hire_date + emp_id`, nor by a natural key of a name and a date - a pair that works is never grown to three, and every key found is minimal by construction. The columns that take part in a combination are the 24 most key-like in table order - an identifier, a name or a date in the name first - and a measure only in a second pass when they found no key at all: a key with an amount in it beats no key, and is a coincidence beside one. A combination whose distinct counts multiply to fewer than the rows cannot be unique and is not counted; the rest are counted the tightest first - every pair, then the 300 tightest combinations of three, then of four - and over 5,000 rows on a random sample of 5,000 rows first: a duplicate there is a duplicate on every row, so only the combinations unique on the sample are verified on every row - a full count each, the tightest first, and a statement that verifies a key ends the level, so a designed key costs one and the coincidences behind it are never counted in full. The counting itself reads one side: every level - every column, then the combinations, the sample first - is counted on the Left alone, and a combination unique there is verified on the Right before it can end the search: unique there too, and sharing values. One that is not is listed all the same and the search goes on to the next level. That is half the counting of measuring both sides at every level, and the same answer. Only keys end a level - a combination unique on both sides with no value in common pairs no rows, so it is listed but ends nothing - and when nothing is unique the closest are listed, each cut back to the fewest columns that tell as many rows apart. On a wide table with no single unique column this takes minutes - see [Speed](#speed). Candidates rank by unique on both sides, then whether they share any values at all - a row number each side counts for itself pairs nothing, so it ranks under a name and a date shared by both, and is no reason to stop looking for a key that pairs - then whether a measure or a checksum is among them - a row hash is unique by construction and pairs nothing but itself, so it reads *a checksum - unique by construction, not a key* and ranks under `emp_id` however unique it is - then whether an identifier is - `emp_id` above a name and a date - then how close it comes - distinct values per row - then overlap, then fewer columns, then how much the names look like a key on average. How close before the names, and the names averaged rather than summed: a sum grows with every column added, so four key-like names outscored the two that were the key whenever nothing was unique on both sides. The search runs in the red disc like every other run and ends on the best - *Best key: emp_id in 0.8s* - with its steps kept in the Log; pick rows from the list and press **Use as key**. When a profile of the same columns exists (Auto's *Profile both sides first*, or **Profile both files** under Rows), the single-column counts come from it and are not measured twice; a column with nulls is penalised and a constant column is skipped.
...
The results show **one view at a time** - a radio above them, not tabs: Streamlit builds every tab on every rerun, so with four tabs each click on the page rebuilt the report and read every result file into memory. Only the view on screen is built.

| View | What it holds |
|---|---|
| Summary | A **Key** block first: the key columns as green chips and the line *Rows are matched on emp_id - 2,960 rows matched* - or *No key - rows were matched by hashing the 5 compared columns*, or *No key - rows were paired by position, line 1 against line 1* - with the warning under it when the key is not unique. Then the row counts; then every column from either file on one sheet - its name on each side, its role (key, compared, paired but not compared, only in Left, only in Right) in the column table's colours, green for the key and red for a column that took no part, how it was read, and for compared columns matched / mismatched / match % - with the one-sided columns called out above it. **Profile by bucket**: the top values for the rows whose keys matched, matched but differ, or exist on one side only - the **key columns** counted, and any other column under *Other columns* when you add it, so a wide pair does not count hundreds of columns nobody asked to see. The matched-but-different bucket opens with **differences by key value** - the rows that paired on the key but disagree, grouped by each key column's value, with the columns that differ. The key is identical on both sides for these rows: this is where the differences sit, not what they are. |
| Columns & values | The rows that differ, Left above Right, differing cells marked. Then the compared columns that differ, worst first - the first 12 open, the rest under *Other columns* - each with the value pairs behind its count (one DuckDB pass over the cell differences). What a column holds across a set of rows is counted in one place, *Profile by bucket* on the Summary. **Near-match analysis** says whether differences are formatting or data. |
...
4. **Profile both sides** - the **Profile both sides first** tick under the button, on by default. Counts, nulls and distinct values per column feed the key search, show under Rows as the Profile, and land in the run folder as `profile.csv`; a constant column, an empty one, or a distinct count that differs more than twice between the sides becomes a note. It measures every column of both files - minutes on a big or wide pair - and Auto finds the key without it, so it is asked for rather than assumed. A profile already taken on the same columns and Types is reused; one taken before Auto changed a Type is measured again, on the typed values.
...
What the values look like decides the types - there is no column table here to take a suggestion in - so a text column of `12,686.95` profiles as a number, `06-Nov-2019` as a date with its format, `Y`/`N` as a boolean; a column DuckDB typed keeps that type. The table is read into DuckDB once and every measure below runs on that one read; only the checks for stray spaces, case variants and leading zeros go back to the file, because they need the value as written. A table too big to hold - more than a quarter of DuckDB's memory limit at some 24 bytes a value, a million rows of two hundred columns on a 16 GB machine - is not read in: the disc says *too big to hold in memory - measured from the file* and every measure reads and types the columns it needs from the file, slower per measure but with no copy of the table in memory, so a table of any size profiles. Every measure that counts distinct values takes the columns a few at a time - 32 a statement, fewer as the rows grow - because each such count holds a hash table and hundreds at once run DuckDB out of memory on a wide table however few the rows. A wide or big table takes minutes rather than seconds - the caption under the button says so, and [Speed](#speed) says where they go. The run shows in the red disc under the switch - *Profiling…*, then *Looking for keys…* and *What stands out…* on the way, ending on *Profile ready - key: emp_id in 0.4s* (or *Profile ready - no key in 0.4s*) - and lands in the Log as two entries: the run with its steps, and *Profile notes* - *8 things stand out* - with one line per note, the way Auto's decisions are logged. The page then reads, top to bottom: **File**, **Profile** with its headline, **Keys**, **Statistics** with the download and save row, **What stands out**, the **Outliers**, **Patterns** and **Dependencies** folds, then **Value frequencies**.
...
**Keys** - a green line when the best candidate is unique on every row: *Key: emp_id - unique on every row. Found by measuring every column - a column unique by itself is the key, so no combination was tried - `pip install desbordante` for exact key discovery (HyUCC / PyroUCC) - single-column figures from the profile.* Otherwise a warning, *Nothing up to 4 columns is unique - the closest are below.*, with the same note after it. The candidate table sits under it: the best three. Beside a key only keys are listed - each minimal, no two the same, none the key with a column added - and when nothing is unique, the three closest. This is the Compare page's **Suggest keys** on one table, a level at a time: every column on its own, then every pair of the 24 most key-like columns, then the 300 tightest combinations of three, then of four - each level only when the one before found no key, so a column unique by itself is never combined with anything and nothing is looked for beside it, a pair that works is never grown, and every key found is minimal by construction - with Desbordante when it is installed (HyUCC on the first 200,000 rows, verified on every row; PyroUCC's almost-unique combinations when nothing is exact). A measure is tried only when the key-like columns found no key at all, and a column with a null is in no combination - a row with no key is never told apart, so it can complete none. On `hr_employees.csv` that is `emp_id`, and the search ends there: the natural key `first_name + last_name + hire_date` is never counted - drop `emp_id`, and it is found among the combinations of three. On a wide table with no single unique column this is the slow part of the profile - minutes, the disc saying which level it is on. The distinct and null counts of the single columns come from the statistics just measured, so nothing is counted twice - that is what the note's last words say - and a column with one value is never a candidate. There is no other side, so no Overlap %; candidates rank by unique first, then whether a measure is among them, then whether an identifier is, then how close - distinct values per row - then fewer columns, then how much the names look like a key on average. The columns:
...
| Looks like a key | `yes` when the names and detected types read as identifiers, `measure columns` when one of them is a measure - an amount, a rate, a decimal - and `checksum` when one is a checksum of the row: named as one (`row_hash`, `md5`, `checksum`, `fingerprint`…) or holding digests, every filled value hex of one digest length (32, 40, 64 or 128). A checksum is unique by construction and says nothing about which row it is, so it is listed under the key and never taken beside one. |
| Why | The reasons in one line, as on the Compare page but naming no side. `emp_id`: *name says identifier · no nulls · 3,000 distinct of 3,000 · unique by itself*. `first_name + last_name + hire_date`, the key once `emp_id` is gone: *a name, not an identifier · no nulls · 3,000 distinct of 3,000 · unique as three - no single column or pair is*. When nothing is unique, the closest: *name says identifier · 10 nulls · 90 distinct of 100 · on its own - with a null it can complete no key* for an `id` column with 10 empty cells - the 10 rows with no id have no key, so they are neither distinct nor duplicates, and the column was in no combination - or *on its own - no other column to add* when there was none to try, *on its own - adding a column told no more rows apart* for a combination cut back to one column, *the closest - no combination up to 4 columns is unique* for one that stayed a combination; *50 distinct of 100 · 50 rows share it* for one where every row appears twice. |
...
|---|---|
| `12 exact duplicate rows - the same values in every column` | Rows that repeat another one in every column, told apart by the MD5 of the row's values, a null equal to a null. |
| `key: emp_id - unique on every row` (HR), or `no single column or combination up to 4 is unique - closest: first_name + last_name (2,998 distinct of 3,000)` | The best candidate from Keys - a key as the search counted it, the closest counted again on the table. |
...
![The Profiling page: the done disc naming the key, the File section, the headline, the Keys block with its candidate table, and the Statistics table](docs/app-profiling.png)
...
- The key search on a pair counts one side and verifies what it finds on the other, so a level costs one side's counting, not two - see [Suggest keys](#key).
- DuckDB gets three quarters of its own default limit - about 60% of the machine - and a temp directory in the work folder, so past that it spills to disk rather than failing and there is room left for Streamlit, pandas and the browser. `COMPARE_DUCKDB_MEMORY` sets the limit outright when the machine is shared, and with it the table the profile and the key search hold in memory.
...
tablecmp/
  ...
  ui_results.py       the verdict and the results page - one view at a time
